...
if (empty($_POST['title']) || empty($_POST['content'])) {
    echo "<script>alert('공지 제목 또는 내용이 비어있습니다.'); history.back()</script>";
    exit();
}
...
$sql = "INSERT INTO Notice(title, content, id, attime, fixattime, file_name, file_path)
    VALUES (?, ?, ?, NOW(), NOW(), ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssss", $title, $content, $id, $file_name, $file_path);
    $stmt->execute();
    echo "<script>window.location.replace('notice_list.php');</script>";
    exit();
